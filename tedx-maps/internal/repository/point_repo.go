...
func (r *pointRepo) Update(ctx context.Context, p *entity.Point) error {
	query := `UPDATE points SET name=$1, latitude=$2, longitude=$3, type=$4, updated_at=NOW() WHERE id=$5`
	_, err := r.db.ExecContext(ctx, query, p.Name, p.Latitude, p.Longitude, p.Type, p.ID)
	return err
}
...
func (r *pointRepo) GetByID(ctx context.Context, id int64) (*entity.Point, error) {
	row := r.db.QueryRowContext(ctx, `SELECT id, name, latitude, longitude, type, created_at, updated_at FROM points WHERE id=$1`, id)
	var p entity.Point
	if err := row.Scan(&p.ID, &p.Name, &p.Latitude, &p.Longitude, &p.Type, &p.CreatedAt, &p.UpdatedAt); err != nil {
		return nil, err
	}
	return &p, nil
}
